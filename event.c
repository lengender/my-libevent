...
//handler signals -   this is a deprecated interface
int (*event_sigcb)(void);   //signal callback when gotsig is set
voliatile sig_atomic_t event_gotsig;
...
int event_base_once(struct event_base* base, int fd, short events,
    void(*callback)(int, short, void*), void* arg, const struct timeval *tv){
        ...
        else if(events & (EV_READ | EV_READ)){
...
timeout_correct(struct event_base *base, struct timeval *tv){
    ...
    prev = base->timeheap.p;
    size = base->timeheap.n;

    for(; size-- > 0; ++prev){
        struct timeval *ev_tv = &(**prev).ev_timeout;
        evutil_timersub(tv_tv, &off, ev_tv);
    }
...
timeout_next(struct event_base* base, struct timeval **tv_p){
    ...
    if((ev = min_heap_top(&base->timeheap)) == NULL){
        //if no time-based events are active wait for IO
        *tv_p = NULL;
        return 0;
    }
...
void timeout_process(struct event_base *base){
    ...
    while((ev = min_heap_top(&base->timeheap))){
        ...
        //delete this event from the IO queue
        event_del(ev);
